...
const fight_k__ = process.env['fight_k__']

let page = '/'

let db = {}

db = FILE.readFileSync('./db_fight_k.json',
  ...
  FETCH(fight_k__ + page)
    ...
    .then(async t => {
      ...
      for (let video of videos)
        await get_video_url(video.attribs.href, video.attribs.title, video.children[0].attribs.src)
...
async function get_video_url(video_url, video_title, video_thumb) {
  ...
  for (let video of videos_found) {
    ...
    let exists = db.fight_k.find(v => v.url == video_url)
    ...
    try {
      ...
      archivo_completo = './media/fight_k/' + video_source.split('/').pop()

      if (Number(peso) > 52428800)
        archivo_completo = './media/fight_k_pesados/' + video_source.split('/').pop()
...
function save_db() {
  FILE.writeFileSync('./db_fight_k.json', JSON.stringify(db, null, 2),
    (err) => {
      if (err) {
